...
$sql = " SELECT 
        ...
        boxnum,
        (
            (CASE WHEN hose1 LIKE ? THEN 1 ELSE 0 END) +
            (CASE WHEN hose2 LIKE ? THEN 1 ELSE 0 END) +
            (CASE WHEN hose3 LIKE ? THEN 1 ELSE 0 END) +
            (CASE WHEN hose4 LIKE ? THEN 1 ELSE 0 END) +
            (CASE WHEN hose5 LIKE ? THEN 1 ELSE 0 END) +
            (CASE WHEN hose6 LIKE ? THEN 1 ELSE 0 END) +
            (CASE WHEN hose7 LIKE ? THEN 1 ELSE 0 END)
        ) AS vencendo
    FROM boxdata
    WHERE boxzone LIKE ?
    HAVING vencendo > 0
...
$param = "%" . $busca . "%";
$zoneparam = "%" . $boxzone . "%";
$stmt->bind_param("ssssssss", $param, $param, $param, $param, $param, $param, $param, $zoneparam);
...
<html lang="pt-br">
  ...
  <body>
    <main class="main-menu">
      ...
      <h2 class="menu-subtitle">Mangueiras Vencendo - <?php echo $_GET['zone'] ?></h2>
      <div class="content">
        <table class="table">
          <tr>
            <th>Nº</th>
         ...
         <?php
              if ($resultado->num_rows > 0) { while ($linha = $resultado->fetch_assoc()) { 
                ...
                echo "<td>" . htmlspecialchars($linha["vencendo"]) . "</td>"; 
                echo "</tr>"; } 
              } else { 
                echo "<tr>"; 
                echo "<td colspan='3'> Nenhuma mangueira vencendo </td>"; 
                echo "</tr>"; 
                } ?>
        </table>
         <?php
